#!/usr/bin/env python3
"""Prepare base-only skillbooks for CAR-bench evaluation (Section 12a).

Extracts skillbook_49.json (state after 50 base traces, before disambiguation/
hallucination contamination) from each variance experiment run.  For each of
4 configs (haiku-nobudget, haiku-500, sonnet-nobudget, sonnet-500):

  1. Loads skillbook_49.json from all 5 runs
...
Usage:
    uv run python scripts/prepare_base_only_eval.py
    uv run python scripts/prepare_base_only_eval.py --dry-run  # Print info only
"""
...
SNAPSHOT_INDEX = 49  # skillbook after 50 base traces (0-indexed)
SIMILARITY_THRESHOLD = 0.7
MIN_RUNS_FOR_STABLE = 3  # skill must appear in >= 3/5 runs

MODELS = {
    "haiku": "variance_experiment_car_haiku_4.5",
    "sonnet": "variance_experiment_car_sonnet_4.6",
}
BUDGETS = ["no-budget", "budget-500"]
RUNS = [1, 2, 3, 4, 5]

RESULTS_DIR = PROJECT_ROOT / "results"
OUTPUT_DIR = RESULTS_DIR / "car_bench_eval_base_only" / "skillbooks"
WIKI_ORIG = (
    PROJECT_ROOT
    / "benchmarks"
    / "car-bench"
...
# ---------------------------------------------------------------------------
# Step 1: Load all base-only skillbooks
# ---------------------------------------------------------------------------


def load_base_skillbooks() -> dict[str, dict[str, dict[int, Skillbook]]]:
    """Load skillbook_49.json for all configs.
        ...
        for budget in BUDGETS:
            ...
            for run_num in RUNS:
                sb_path = (
                    ...
                    / budget
                    / f"run_{run_num}"
                    / f"skillbook_{SNAPSHOT_INDEX:02d}.json"
                )
                if not sb_path.exists():
                    ...
                    sb_path = (
                        ...
                        / budget
                        / f"run_{run_num}"
                        / f"skillbook_{SNAPSHOT_INDEX}.json"
                    )
                if not sb_path.exists():
...
def save_metadata(
    ...
    meta_path = OUTPUT_DIR / "base50_metadata.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare base-only skillbooks for CAR-bench evaluation"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    print("=" * 70)
    print("Base-Only Skillbook Preparation (Section 12a)")
    print(f"  Snapshot: skillbook_{SNAPSHOT_INDEX:02d}.json (50 base traces)")
    print(f"  Output:   {OUTPUT_DIR}")
    print("=" * 70)

    # Step 1: Load
    print("\n--- Step 1: Loading base-only skillbooks ---")
    data = load_base_skillbooks()
