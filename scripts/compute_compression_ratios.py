...
For each of the 35 runs (7 budgets x 5 runs):
  ...
  - % of Original: (compressed / original) * 100

Prints per-run data and a summary table grouped by budget.

Usage:
    uv run python scripts/compute_compression_ratios.py
    uv run python scripts/compute_compression_ratios.py results/variance_experiment_sonnet_4.6
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute per-run compression ratios")
    parser.add_argument(
        "base_dir",
        nargs="?",
        default="results/variance_experiment_haiku_4.5",
        help="Experiment base directory (default: results/variance_experiment_haiku_4.5)",
    )
    ...
    print("=" * 90)
    print(
        f"{'Budget':<15} {'Run':>4} {'Original MD':>14} {'Compressed MD':>15} {'% of Original':>15}"
    )
    print("-" * 90)
    for r in rows:
        print(
    ...
    print()
    print("Note: '% of Original' = (compressed_tokens / original_tokens) * 100")
    print("      Lower % means more compression (e.g. 45% = compressed to 45% of original).")
